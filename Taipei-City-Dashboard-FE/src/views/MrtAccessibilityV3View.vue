...
const isMapView = computed(() => route.name === "mrt-a11y-v3-mapview");

// ── 站點 layer：reuse 既有 mrt_station_demo.geojson ──────────────────────────
const STATION_LAYER = {
...
// ── Component configs（chart_data 初始 null，等 fetch 回來填）────────────────
const c1Component = ref({
	id: "mrt-a11y-v3-alert-count",
	index: "mrt_a11y_v3_alert_count",
	city: "taipei",
	name: "C1｜目前異常設施總數",
	source: "BE Mock｜/api/v1/mrt/a11y/alert-count",
	time_from: "current",
	time_to: null,
	update_freq: 15,
...
const c2Component = ref({
	id: "mrt-a11y-v3-alert-by-line",
	index: "mrt_a11y_v3_alert_by_line",
	city: "taipei",
	name: "C2｜各捷運線異常站數",
	source: "BE Mock｜/api/v1/mrt/a11y/alert-by-line",
	time_from: "current",
	time_to: null,
	update_freq: 15,
	update_freq_unit: "minute",
...
const c3Component = ref({
	id: "mrt-a11y-v3-alert-by-type",
	index: "mrt_a11y_v3_alert_by_type",
	city: "taipei",
	name: "C3｜異常設施類型分布",
	source: "BE Mock｜/api/v1/mrt/a11y/alert-by-type",
	time_from: "current",
	time_to: null,
	update_freq: 15,
	update_freq_unit: "minute",
...
const c4Component = ref({
	id: "mrt-a11y-v3-station-overview",
	index: "mrt_a11y_v3_station_overview",
	city: "taipei",
	name: "C4｜捷運站無障礙狀態總覽",
	source: "BE Mock｜/api/v1/mrt/a11y/station-overview",
	time_from: "current",
	time_to: null,
	update_freq: 15,
	update_freq_unit: "minute",
...
// ── fetch + transform（依 .claude/skills/page 的 chart-data 格式對應） ─────────
async function fetchAll() {
	...
	// C3 N×1 series → DonutChart（攤平成 2D）
...
onMounted(async () => {
	contentStore.currentDashboard.name = "雙城暢行 V3 ｜捷運無障礙即時狀態";
...
<template>
  <!-- 儀表板總覽：4 個 component grid -->
  <div
    v-if="!isMapView"
    class="mrtaccessibilityv3view-overview"
  >
    <DashboardComponent
      :config="c1Component"
      mode="default"
  ...
  <div
    v-else
    class="mrtaccessibilityv3view-mapview"
  >
    <div class="mrtaccessibilityv3view-mapview-charts">
      <h2>無空間資料組件</h2>
      <DashboardComponent
        :config="c1Component"
        mode="map"
      ...
      />
      <p class="mrtaccessibilityv3view-mapview-charts-tip">
        打開 C4 開關可在地圖載入站點圖層；
        <br>紅點＝有異常、綠點＝正常。
      </p>
    </div>
