...
在 `src/router/index.js` 的 `routes` 陣列中新增路由：
...
在 `src/router/index.js` 中，找到處理內容載入的 `router.beforeEach`（約第 164 行），加入新路由的資料載入邏輯：
...
router.beforeEach((to) => {
  ...
  if (
    ...
    to.path.toLowerCase() === "/my-custom"        // ← 新增
  ) {
...
### 步驟四：在 App.vue 中配置 layout

在 `src/App.vue` 的 template 中，為新頁面設定對應的 layout 結構（SideBar、SettingsBar 等）。參考既有的 dashboard/mapview 條件渲染邏輯。
